...
// Each stage requires 100% correct to pass (same as your example)
...
export default function CumulativeQuiz() {
  ...
  const handleLoadData = async () => {
    try {
      ...
      // 1) Load chapters_demo for this book + user
      ...
      // 6) Combine data => mark isRead, stageStatus, attach readingRecords, etc.
      tempChapters.forEach((ch) => {
        ch.subchapters.forEach((sub) => {
          ...
          // For each stage => compute "Not Started", "WIP", "Completed"
          sub.stageStatus = {};
          sub.quizRecords = {};
          sub.revisionRecords = {};
      ...
      setStatus("Data loaded successfully! Found " + tempChapters.length + " chapters.");
    } catch (err) {
  ...
  return (
    <div style={styles.container}>
      <h1>Cumulative Quiz (Comprehensive)</h1>
      <p style={{ marginBottom: "1rem" }}>
        This mirrors the tabular approach from <b>AdaptivePlanLoader</b> but in a dedicated Cumulative Quiz component.
      </p>

      {/* Inputs for Book/Plan/User IDs */}
      ...
      {/* Render each chapter in a table */}
...
/** computeStageStatus => if no quiz => "Not Started", else check pass/fail of latest quiz */
function computeStageStatus(quizArr, revArr, stage) {
  if (!quizArr.length) {
    return "Not Started";
  }
  const [latestQuiz] = quizArr; // sorted desc => newest attempt
...
/** parseScoreForRatio => identical to your example */
...
/** ChapterSection => same approach as in AdaptivePlanLoader, with expand/collapse, table of subchapters */
...
/** renderStageCell => "Not Started"/"WIP"/"Completed" + info button for timeline */
...
/** InfoPopover => same idea as your example: a small "i" button that shows details. */
...
/** renderStageTimeline => merges quiz + revision attempts in ascending attemptNumber */
function renderStageTimeline(quizArr, revArr, stage) {
  ...
  ascQuizzes.forEach((quizDoc) => {
    ...
    // see if there's a matching revision with same attemptNumber
