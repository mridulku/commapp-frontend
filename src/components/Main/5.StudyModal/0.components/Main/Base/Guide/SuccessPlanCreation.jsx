/* ────────────────────────────────────────────────────────────────
   File:  src/components/GuideCarousel.jsx
   “Your plan is ready” summary slide
   • Pure presentational – give it a `planDoc` prop or it falls back
     to the MOCK_PLAN below (handy for Storybook / local testing)
   • No Redux imports; mount / un-mount from any parent.
───────────────────────────────────────────────────────────────── */
...
import MoreHorizIcon   from "@mui/icons-material/MoreHoriz";  /* ← for the “+ n” pill */
import {
  Box,
  Typography,
  Chip,
...
async function handleStart() {
  ...
  try {
    ...
    dispatch(setCurrentIndex(oldIndex + 1));    // ➜ next activity
  } catch (err) {
  ...
  /* topics – inline if fits, else chips */
  const TopicsCovered = () => {
    ...
    if (flatText.length <= 110)
      return (
        <Typography
          variant="body2"
          sx={{ color:"#ccc", fontWeight:500, mb:2, lineHeight:1.4 }}
        >
          {flatText}
        </Typography>
      );
          ...
          <Box key={subj} sx={{ display:"flex", alignItems:"center", gap:1, flexWrap:"wrap" }}>
            ...
            {Object.keys(groups).map((g) => (
              <Chip
                ...
                sx={{
                  bgcolor:ACCENT,
                  "& .MuiChip-label":{ fontWeight:500, color:"#000" },
                }}
              />
            ))}
          </Box>
  ...
  /* first-three-sessions preview */

  /* ──────────  prettier 3-day glance  ────────── */
/* ───────────── cleaner list style  ───────────── */
const SessionsPreview = () => {
  ...
  const META = {
    ...
    QUIZ: {
      chip: (a) =>
        `${a.quizStage?.charAt(0).toUpperCase() + a.quizStage?.slice(1)} Quiz`,
      icon: <QuizIcon sx={{ fontSize: 16, color: "#000" }} />,
      color: "#FFD54F",
    },
  };

  /* helper – one row per activity */
  const ActivityRow = (a, idx) => {
    ...
    return (
      ...
      >
        ...
        {/* full sub-chapter name, ellipsised only if crazy-long */}
        <Typography
          variant="body2"
          sx={{
            whiteSpace: "nowrap",
          ...
          title={a.subChapterName} // tooltip on hover
        >
          {a.subChapterName}
        </Typography>
      </Box>
