 ...
 *   - If reading => reading completion date, total time, day-based lumps
 *   - If quiz => for each Q or R => submission date, day-based lumps, concept pass/fail
 */
export default function CompletionSummaryPanel({
  activity,
  aggregatorObj,
  conceptList = [],
  attempts = [],  // attempts array if you merged quiz+revision
}) {
  ...
  // aggregator => readingSummary or quizStagesData
...
function renderReadingSummary(readingSummary, timeData, loading, err) {
  const completionDateStr = readingSummary?.completionDate
    ? new Date(readingSummary.completionDate.seconds*1000).toLocaleDateString()
    : "(not completed)";
  ...
  return (
    <Box>
      ...
      <Typography variant="body2">Completion Date: {completionDateStr}</Typography>
...
function renderQuizSummary(aggregatorObj, conceptList, attempts, timeData, loading, err) {
  const allAttempts = attempts; // the same "combined" array with timestamp, attemptNumber, etc.
  ...
  return (
    <Box>
      ...
      {allAttempts.map((att, idx)=>{
        ...
        // doc-level lumps => day-based usage
        const lumpsArr = buildDayBreakdown(timeData?.details||[], label, att);

        // aggregator => concept pass/fail => or question pass/fail
        ...
        return (
          <Box key={idx} sx={{ mb:2, p:1, backgroundColor:"#444", borderRadius:1 }}>
            ...
            {/* concept wise pass/fail => compare aggregator concepts vs quizSubmission */}
