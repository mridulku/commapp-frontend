...
export default function AdaptivePlanGrid() {
  ...
  return (
    <div style={styles.chapterContainer}>
      <div style={styles.chapterHeader} onClick={() => setExpanded(!expanded)}>
        <h2 style={{ margin: 0 }}>{expanded ? "▼" : "▶"} {chapter.chapterName}</h2>
      </div>
      {expanded && (
...
function LockedStage() {
  return (
    <div style={styles.lockedBox}>
      <span role="img" aria-label="lock">🔒</span> Locked
    </div>
  );
}

// --------------------------------------
// The small custom modal to show timeline attempts
// --------------------------------------
function StageModal({ subchapter, stageName, timeline, onClose }) {
  return (
...
/**
 * AttemptDetail => shows e.g. "Q: Quiz 1 (Fail)", then lists concepts tested
 */
function AttemptDetail({ attempt }) {
  const prefix = attempt.actionType === "quiz" ? "Q:" : "R:";
  let suffix = "";
  switch (attempt.result) {
    case "pass": suffix = " (Pass)"; break;
    case "fail": suffix = " (Fail)"; break;
    case "done": suffix = " (Done)"; break;
    case "not-started": suffix = " (Not Started)"; break;
    default: suffix = "";
  }
      ...
      <div style={{ marginLeft: "16px" }}>
        {attempt.conceptsTested && attempt.conceptsTested.length > 0 ? (
          attempt.conceptsTested.map((c, i) => {
            let conceptResult = c.pass === true ? "Pass" : c.pass === false ? "Fail" : "Not Tested";
            return (
