...
export default function OnboardingModal({ open, onClose }) {
  ...
  async function handleUserAnswer(answer) {
    ...
    console.log(
      `[handleUserAnswer] stepIndex=${stepIndex}, type=${step.type}, field=${step.field}, answer=${answer}`
    );
    ...
    if (step.field === "confirmFinish") {
      if (answer.startsWith("Yes")) {
        ...
        // "No, go back" => go back one step for simplicity
    ...
    if (nextIndex < steps.length) {
      ...
      console.log(
        `Moving to stepIndex=${nextIndex}, type=${nextStep.type}, field=${nextStep.field}`
      );
      ...
      // If nextIndex == steps.length => we got to the end
...
/** Basic modal overlay + container styles below... */
const overlayStyle = {
  position: "fixed",
  top: 0,
  left: 0,
